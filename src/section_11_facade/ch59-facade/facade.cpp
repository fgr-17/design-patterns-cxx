...
class Window {
   public:
    explicit Window(const std::string title, uint8_t columnCount, uint8_t columnWidth,
                    uint8_t charHeight)
    ...
    void draw() {
        std::cout << "Drawing window" << title_ << " with " << columnCount_ << " columns, "
                  << columnWidth_ << " width and " << charHeight_ << " height" << std::endl;
    }
...
class Console {
   public:
    ...
    std::shared_ptr<Window> multicolumn(const std::string& title, uint8_t columnCount,
                                        uint8_t columnWidth, uint8_t charHeight) {
...
int main() {
    static constexpr uint8_t columnCount = 3;
    static constexpr uint8_t columnWidth = 10;
    static constexpr uint8_t charHeight = 15;
    printTitle();
    Console::instance().multicolumn("Test", columnCount, columnWidth, charHeight);
    return 0;
}
