...
struct AddressFactory {
    static std::unique_ptr<Address> newAddress(std::string street, std::string city,
                                               const int& suite) {
...
struct Contact2 {
    ...
    Contact2(std::string name, Address* address)
        : name(std::move(name)), address(address ? std::make_unique<Address>(*address) : nullptr) {}
...
struct EmployeeFactory {
    static std::unique_ptr<Contact2> newMainOfficeEmployee(const std::string& name,
                                                           const int suite) {
   ...
   private:
    static std::unique_ptr<Contact2> newEmployee(const std::string& name, const int suite,
                                                 const Contact2& prototype) {
