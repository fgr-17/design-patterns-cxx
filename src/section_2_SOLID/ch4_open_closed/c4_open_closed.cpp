...
struct ProductFilter {
    ...
    static std::vector<Product*> bySizeAndColor(const std::vector<Product*>& items,
                                                const Color color, const Size size) {
        std::vector<Product*> result;
        for (const auto& i : items) {
    ...
    static int printOutput(const std::vector<Product*>& items) {
        for (const auto& i : items) {
            std::cout << i->name << " - color: " << colorString.at(i->color)
                      << " - size: " << sizeString.at(i->size) << std::endl;
        }
...
int main() {
    ...
    for (auto& item : bf.filter(items, green)) std::cout << item->name << " is green" << std::endl;
